...
fd <- dcast(fudan_guangzhou[!grep("Guangzhou", Group)], Pathway~sampleID, value.var = "abundance_ra") # sample in column
fd <- dt.to.matrix(fd)
...
oFD_model <- filter.features(oFD, filter.method = 'abundance', cutoff = 0.001)
...
yFD_model <- filter.features(yFD, filter.method = 'abundance', cutoff = 0.001)
