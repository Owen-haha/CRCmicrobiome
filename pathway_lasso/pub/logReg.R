library(data.table) #, lib.loc="/hwfssz1/ST_HEALTH/P18Z10200N0127/yqin/bin/lib/R4.0")
...
pub <- fread("/hwfssz1/ST_HEALTH/P18Z10200N0127/yqin/crczz/fudan/modelling/data_ml/pathway_pub_data_n1260.tsv")

pub_ra <- dcast(pub, Pathway~sampleID, value.var = "abundance_ra") # sample in column
pub_ra <- dt.to.matrix(pub_ra)

pub_phe <- unique(pub[, .(sampleID, study_condition, age, gender, tnm, dataset_name)]) # sample in row
pub_phe <- dt.to.matrix(pub_phe)
pub_phe <- as.data.frame(pub_phe)

# guangzhou
oGZ <- dcast(fudan_guangzhou[Group=="oCRC_Guangzhou"], Pathway~sampleID, value.var = "abundance_ra") # sample in column
oGZ <- dt.to.matrix(oGZ)

yGZ <- dcast(fudan_guangzhou[Group=="yCRC_Guangzhou"], Pathway~sampleID, value.var = "abundance_ra") # sample in column
yGZ <- dt.to.matrix(yGZ)

oFD <- dcast(fudan_guangzhou[Group=="oCRC_Fudan" | Group=="oControl"], Pathway~sampleID, value.var = "abundance_ra") # sample in column
...
yFD <- dcast(fudan_guangzhou[Group=="yCRC_Fudan" | Group=="yControl"], Pathway~sampleID, value.var = "abundance_ra") # sample in column
...
sc.obj <- siamcat(feat=pub_ra, meta=pub_phe, label='study_condition', case='CRC')

sc.obj <- filter.features(sc.obj, filter.method = 'abundance', cutoff = 0.001)

sc.obj <- normalize.features(sc.obj, norm.method = "log.std",  
                             norm.param = list(log.n0 = 1e-08, n.p = 2, sd.min.q = 0.1, norm.margin = 1))
