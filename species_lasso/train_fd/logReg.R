...
fd <- dcast(fudan_guangzhou[!grep("Guangzhou", Group)], Taxon~sampleID, value.var = "abundance_ra") # sample in column
fd <- dt.to.matrix(fd)
...
cat("cross validation of oFD:\n")
oFD_model <- make.predictions(oFD_model)
oFD_model <-  evaluate.predictions(oFD_model)
#model.evaluation.plot(oFD_model) # 10/5/2 folds validation Average AUC: 0.654/0.689/0.768
oFD_model
